<script lang="ts">
  ...
  function onMouseDown(e: MouseEvent) {
    ...
    if (hit) {
      ...
      isPanning = true; panSX = e.clientX; panSY = e.clientY; panOX = tx; panOY = ty;
    }
  }
  ...
  function linkX(endpoint: string | GNode): number { return (endpoint as GNode).x ?? 0; }
  function linkY(endpoint: string | GNode): number { return (endpoint as GNode).y ?? 0; }
  function linkId(endpoint: string | GNode): string { return typeof endpoint === 'string' ? endpoint : endpoint.id; }
    ...
    const ro = new ResizeObserver(() => {
      readSize();
      sim?.force('center', d3.forceCenter(width / 2, height / 2).strength(0.12));
    ...
    sim = d3.forceSimulation<GNode>(nodes)
      ...
      .force('radial',    d3.forceRadial<GNode>(
                            d => d.degree === 0 ? 80 : 0,
                            width / 2, height / 2
