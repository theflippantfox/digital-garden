<script lang="ts">
  ...
  $: note = data.note;
  $: accent = ACCENT_STYLES[note.accent as Accent];
  // Layout data (all notes) is merged from parent loader
  $: allNotes =
    (data as unknown as { notes?: (typeof data.note)[] }).notes ?? [];
...
<div class="flex gap-0 xl:gap-8 xl:pr-8 py-8 md:py-12 min-h-full">
  <!-- ── Main article ───────────────────────────────────────────── -->
  <article class="flex-1 min-w-0 px-5 md:px-10 xl:pl-10 xl:pr-0 max-w-[720px]">
    ...
    {#if note.backlinks?.length}
      <section class="mt-14 xl:hidden" aria-label="Backlinks">
        <h2
